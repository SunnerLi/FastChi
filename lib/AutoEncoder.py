...
class AutoEncoder(object):
    ...
    def conv(self, layer, n_filter=32, filter_size=(3, 3), strides=(1, 1), act = tf.identity, name = 'conv2d'):
        """
        ...
        network = tl.layers.Conv2d(network, n_filter=int(channel / 2), filter_size=filter_size, strides=strides, act = tf.identity, name = name)
        network = tl.layers.BatchNormLayer(network, act = act, name =name + 'batchnorm_layer')
        network = tl.layers.Conv2d(network, n_filter=n_filter, filter_size=(1, 1), strides=(1, 1), act = None, name = name + '_expand')
        return network

    def deconv(self, layer, n_out_channel=32, filter_size=(3, 3), strides=(1, 1), act = None, name = 'deconv2d'):
        """
            The wrapper of inception of convolution
        """
        # Define shape
        ...
        network = tl.layers.DeConv2d(network, n_out_channel = int(channel / 2), filter_size=filter_size, out_size=deconv_shape, strides = strides, act = tf.identity, name =name)
        network = tl.layers.BatchNormLayer(network, act = act, name = name + 'batchnorm_layer')
    ...
    def residual_block(self, layer, n_filter=64, filter_size = (3, 3), name = 'res_block'):
        tmp = self.conv(layer, n_filter=n_filter, act = lrelu, name = name + 'conv2d_1')
